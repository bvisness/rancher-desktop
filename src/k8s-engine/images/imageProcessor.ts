    ...
    return await new Promise((resolve, reject) => {
      ...
      child.on('exit', (code, signal) => {
        if (result.stderr) {
          ...
          let cr = result.stdout ? "\r\n" : "";
          if (args) {
            console.log(`> ${ this.processorName } ${ subcommandName } ${ args.join(' ')}:${cr}${ result.stdout }`);
          } else {
            console.log(`> ${ this.processorName } ${ subcommandName }:${cr}${ result.stdout }`);
          }
        }
        if (code === 0) {
          if (sendNotifications) {
